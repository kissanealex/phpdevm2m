...
class CountryDetailsModel
{
    ...
    private function do_get_message_data()
    {
        ...
        if ($this->c_obj_soap_client_handle)
        {
            try
            {
                ...
                {
                    $m_arr_messages = $this->c_obj_soap_client_handle->peekMessages('16rajatt','Assignment16',2147483647,$this->c_message_id);
                }
                else
                {
                    $m_arr_messages = $this->c_obj_soap_client_handle->peekMessages('16rajatt','Assignment16',2147483647);
                }
    ...
    private function do_parse_downloaded_message_data()
    {
        ...
        {
            $m_obj_parser = new XmlParser();                                   //if not then the parse it to XML format
